...
export class InternetService {
   ...
   */
  dataBundleList(
    transDto: BundleListDto,
  ): Observable<AxiosResponse<InternetDto>> {
    const { network } = transDto;
    console.log('transDto network', transDto);
    
    // Convert string to number if needed and validate network parameter
    ...
    const dblURL =
      this.DataUrl + `/TopUpApi/dataBundleList?network=${networkId}`;
    ...
    this.logger.log(
      `DATA BUNDLE LIST payload config ==> ${JSON.stringify(configs.url)}`,
    );
    return this.httpService
      .get(configs.url, { httpsAgent: configs.agent, headers: configs.headers })
      .pipe(

...
export class MobileMoneyService {
  ...
  async processTransaction(transData: any): Promise<any> {
    ...
    const ptParams: any = {
      ...
      processing_code:
        PROCESSING_CODE_DEBIT || process.env.PROCESSING_CODE_DEBIT,
    ...
    try {
      ...
      const response: AxiosResponse<any> = await firstValueFrom(
        this.httpService.post(configs.url, configs.data, {
  ...
  private async saveOrUpdateTransaction(responseData: any): Promise<void> {
    ...
    switch (responseData.status) {
      case 'Approved':
        ...
        await this.transactionService.updateByTrxn(
          dwParamSave.paymentTransactionId,
          dwParamSave,
        );
        break;
      ...
      default:
        this.logger.warn(
          `Unhandled transaction status: ${responseData.status}`,
        );
        break;
    }
  }
  ...
  private handleError(error: any): void {
    const errorMessage =
      error.response?.data?.reason || error.message || 'Unknown error occurred';
    const statusCode = error.response?.status; // Capture the status code
    // Log the entire error response for debugging
    console.error(
      `Transaction failed: ${JSON.stringify(error.response?.data)} (Status Code: ${statusCode})`,
    );
    throw new Error(
      `Transaction failed: ${errorMessage} (Status Code: ${statusCode})`,
    );
  }
}
